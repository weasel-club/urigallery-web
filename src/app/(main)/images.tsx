...
export default function Images() {
  ...
  const groupImagesByWeek = (images: Image[]) => {
    const map = new Map<string, Image[]>();
    for (const image of images) {
      const date = moment(image.createdAt);
  ...
  const [imageDialogPlaceholder, setImageDialogPlaceholder] = useState<
    Map<string, Blob>
  >(new Map());
  const onImageLoad = useCallback((image: Image, blob: Blob) => {
    setImageDialogPlaceholder((prev) => {
      const newMap = new Map(prev);
      newMap.set(image.name, blob);
      return newMap;
    });
  }, []);
    ...
    images.length > 0 && (
      <div>
        <ImageDialog
          ...
          placeholder={
            displayingImage
              ? imageDialogPlaceholder.get(displayingImage.name)
              : undefined
          }
        />

        {Array.from(imagesByDate.entries())
          .slice(0, showingImageKeys)
          .map(([key, images], i) => (
            <Gallery
              key={key}
              title={key}
